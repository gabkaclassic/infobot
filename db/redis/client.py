...
class PaymentManager:
    ...
    async def create_payment(self, client_id: str, payment_id: str, confirmation_url: str) -> str:
        async with self.payments.redis.client.pipeline(transaction=True) as pipe_payments:
            pipe_payments.set(payment_id, client_id)

            async with self.users.redis.client.pipeline(transaction=True) as pipe_users:
                pipe_users.set(client_id, json.dumps({'confirmation_url': confirmation_url, 'paid': False}))

                results_payments, results_users = await asyncio.gather(
                    pipe_payments.execute(),
                    pipe_users.execute()
                )
